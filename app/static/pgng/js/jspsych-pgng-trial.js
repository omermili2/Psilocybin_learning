  ...
  const info = {
   name: 'pgng-trial',
    ...
    parameters: {
      ...
      outcome_sec_color: {
        type: jspsych.ParameterType.HTML_STRING,
        pretty_name: 'Outcome color for secondary color if needed',
        default:'#000000',
        description: 'Color of outcome text.'
      },
      correct: {
    ...
    new_html += '<div class="score" id="outcome" dir="rtl"></div>';
    new_html += '</div></div>';
    ...
    var present_feedback = function() {
      ...
      var outcome_color_to_present;
      if (trial.outcome_sec_color == '#000000')
        {trial.outcome_sec_color = trial.outcome_color};

      // Define outcome
      if (response.accuracy == 1) {
        trial.outcome = trial.outcome_correct;
        outcome_color_to_present = trial.outcome_color;
      } else {
        trial.outcome = trial.outcome_incorrect;
        outcome_color_to_present = trial.outcome_sec_color;
      ...
      document.getElementById("outcome").style['color'] = outcome_color_to_present;
