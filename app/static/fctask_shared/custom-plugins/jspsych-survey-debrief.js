...
jsPsych.plugins['survey-debrief'] = (function() {
  ...
  plugin.info = {
    ...
    parameters: {
      repeat: {
        ...
        pretty_name: 'Repeat',
        default: true,
        discription: 'If true, include custom task repetition item.'
      },
      mental_demand: {
        type: jsPsych.plugins.parameterType.BOOL,
      ...
      rushing: {
        ...
        pretty_name: 'Rushing',
        default: true,
        discription: 'If true, include the custom rushing to finish item.'
      },
      strategy: {
        type: jsPsych.plugins.parameterType.BOOL,
  ...
  plugin.trial = function(display_element, trial) {
    ...
    if (trial.repeat) {
      ...
      html += '<p>Have you completed this task before today?</p>';
      html += '</div>';

      // iteratively add response options
      html += '<div class="survey-debrief-item" type="response" style="flex-direction: row; align-items: center; justify-content: start;">';
      ['Yes', 'No'].forEach((k, v) => {
        html += '<div>'
        html += '<input type="radio" style="margin: 0 6px 0 ' + (v == 0 ? '40px' : '15px') + '; height: 15px; width: 15px;" name="repeat" id="' + (1-v) + '" value="' + (1-v) + '" required>';
        html += '<label for="' + (1-v) + '" style="font-size: 15px;">' + k +'</label>';
        html += '</div>'
      });
      html += '</div>';
    ...
    if (trial.rushing) {
      ...
      // add prompt
      html += '<div class="survey-debrief-item" type="prompt">';
      html += '<p>How much did you rush or hurry to finish the task?</p>';
      html += '</div>';
      ...
      ['Not at all', '&nbsp;', '&nbsp;', '&nbsp;', '&nbsp;', '&nbsp;', 'Very much'].forEach((k, v) => {
        html += '<div class="radiogroup">';
        html += '<label for="rushing">' + k + '</label>';
        html += '<input type="radio" name="rushing" value="' + v + '" required>';
        html += '</div>';
      });
      html += '</div>';
