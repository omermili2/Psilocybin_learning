 ...
 * 
 * modified to save all attempts - courtesy of code from Yongjing Ren
 * 
 * to 7.2.1 Gili Karni
 */
...
var jsPsychComprehensionsCheck = (function (jspsych) {
  'use strict';
  ...
  const info = {
    ...
    parameters: {
      ...
      correct_required: {
        type: jspsych.ParameterType.BOOL,
        pretty_name: 'Correct required',
        default: true,
        description: 'If true, all answers must be correct.'
      },
    }
  }
  ...
  class ComprehensionCheckPlugin {
    ...
    function doComprehensionCheck(){
      ...
      for (var i = 0; i < trial.questions.length; i++) {
        ...
        html += '<div dir="rtl" id="jspsych-survey-multi-choice-'+question_id+'" class="'+question_classes.join(' ')+'"  data-name="'+question.name+'">';
      ...
      document.querySelector('form').addEventListener('submit', function(event) {
        ...
        rt_arr.push(response_time);
        responses_arr.push(JSON.stringify(question_data));
        qorder_arr.push(JSON.stringify(question_order));

        // save data
        var trial_data = {
          "rt": rt_arr,
          "responses": responses_arr,
          "question_order": qorder_arr,
        };
        ...
        if (all_correct || !trial.correct_required){
