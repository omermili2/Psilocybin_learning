...
var jsPsychOneAlienPractice = (function (jspsych) {
  'use strict';

  const info = {

    name: "one-alien-practice",
    description: '',
    parameters: {
      outcomes: {
  ...
  class OneAlienPracticePlugin {
    ...
    // Draw one aliens
    new_html += `<div class="alien" id="alien-0" state="2" side="0" style="display: none;">`;
    new_html += `<img id="alien-img" src="${trial.aliens[0]}"></img>`;
    new_html += '</div>';
    new_html += `<div class="diamond" id="diamond-0" state="1" side="0"></div>`;
    new_html += `<div class="rock" id="rock-0" state="1" side="0"></div>`;
    ...
    var after_second_response = function(info) {
      ...
      response.state_2_choice = response.state_2_key;
    ...
    var end_trial = function() {
      ...
      jsPsych.setProgressBar(curr_progress_bar_value + (1/trial.outcomes.length));
  ...
  OneAlienPracticePlugin.info = info;

  return OneAlienPracticePlugin;
