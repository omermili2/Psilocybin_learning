 ...
 * Sam Zorowitz
 *
 * plugin for running the comprehension check for the mrst task
 *
 **/
 ...
 var jsPsychMRSTComprehension = (function (jspsych) {
  ...
  class MRSTComprehensionPlugin {
    ...
    // Show preamble text
    html += `<div class="comprehension-box" style="text-align:center">`;
    html += `<div class="jspsych-survey-multi-choice-preamble" style="text-align:center">${quiz_00}</div>`;
    ...
    for (let i = 0; i < trial.prompts.length; i++) {
      ...
      // Add question text
      html += `<p class="jspsych-survey-multi-choice-text survey-multi-choice" style="color:black;">${trial.prompts[i]}</p>`;
      ...
      for (let j = 0; j < trial.options[i].length; j++) {
        ...
        html += `<label class="jspsych-survey-multi-choice-text" for="jspsych-survey-multi-choice-response-${i}-${j}" style="color:black;">${trial.options[i][j]}</label>`;
        html += '</div>';
